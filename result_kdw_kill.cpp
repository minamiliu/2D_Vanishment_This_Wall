...
RESULT_KDW_KILL	g_resultKill[MAX_PLAYER]; //scoreのワーク
...
HRESULT InitResultKDWKill()
{
	...
	D3DXCreateTextureFromFile(pDevice,				// デバイスへのポインタ
								...
								&g_pTextureResultKDWKill);	// 読み込むメモリー
...
HRESULT MakeVertexResultKDWKill(LPDIRECT3DDEVICE9 pDevice)
{
	if(FAILED(pDevice->CreateVertexBuffer(
		...
		D3DPOOL_MANAGED, 
		&g_pVtxBufferResultKDWKill,			//頂点バッファインターフェースのポインタ
		NULL)))
	{
		return E_FAIL;
	}
...
void ChangeResultKDWKill(int nPlayerIdx, int value)
{
	...
	if(g_resultKill[nPlayerIdx].score >= MAX_NUM_BOUND) g_resultKill[nPlayerIdx].score = MAX_NUM_BOUND; //2桁の最大値
