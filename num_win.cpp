...
#define	TEXTURE_POLYGON				"data/TEXTURE/BAR/win_circle.png"// 読み込むテクスチャファイル名
#define	POLYGON_POS_X				(0)								// ポリゴンの基準位置(Ｘ座標)
#define	POLYGON_POS_Y				(60)							// ポリゴンの基準位置(Ｙ座標)
#define	POLYGON_SIZE_X				(25)							// ポリゴンの幅
#define	POLYGON_SIZE_Y				(25)							// ポリゴンの高さ
...
* プロトタイプ宣言
*******************************************************************************/
HRESULT MakeVertexNumWin(LPDIRECT3DDEVICE9 pDevice);
//void SetVertexScore(void);
void SetTextureNumWin( int nPlayerIdx, int nIdx, int number);
...
NUM_WIN	g_numWin[MAX_PLAYER]; //scoreのワーク
...
HRESULT InitNumWin()
{
	...
	//P1 life
	g_numWin[0].pos = D3DXVECTOR3( 1020, 25, 0.0f);
	g_numWin[0].col = D3DCOLOR_RGBA(255,0,0,255); //red 

	//P2 life
	g_numWin[1].pos = D3DXVECTOR3( 1160, 25, 0.0f);
	g_numWin[1].col = D3DCOLOR_RGBA(255,255,0,255); //yellow

	//P3 life
	g_numWin[2].pos = D3DXVECTOR3( 1020, 65, 0.0f);
	g_numWin[2].col = D3DCOLOR_RGBA(0,255,0,255); //green

	//P4 life
	g_numWin[3].pos = D3DXVECTOR3( 1160, 65, 0.0f);
	g_numWin[3].col = D3DCOLOR_RGBA(0,0,255,255); //blue


	//人数分のみ、初期化
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		g_numWin[nCntPlayer].size = D3DXVECTOR3( POLYGON_SIZE_X, POLYGON_SIZE_Y, 0.0f);
		g_numWin[nCntPlayer].score = 0;
	}


	//頂点情報の作成
	if(FAILED(MakeVertexNumWin(pDevice)))
	{
		return E_FAIL;
	}
...
HRESULT MakeVertexNumWin(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		for(int nCntDigit = 0; nCntDigit < NUM_DIGIT; nCntDigit++, pVtx += NUM_VERTEX)
		{
			...
			//color
			pVtx[0].col = g_numWin[nCntPlayer].col;
			pVtx[1].col = g_numWin[nCntPlayer].col;
			pVtx[2].col = g_numWin[nCntPlayer].col;
			pVtx[3].col = g_numWin[nCntPlayer].col;

			// 頂点座標の設定
			pVtx[0].pos = D3DXVECTOR3(g_numWin[nCntPlayer].pos.x  + nCntDigit*POLYGON_SIZE_X, g_numWin[nCntPlayer].pos.y , 0.0f);
			pVtx[1].pos = D3DXVECTOR3(g_numWin[nCntPlayer].pos.x  + g_numWin[nCntPlayer].size.x +  nCntDigit*POLYGON_SIZE_X, g_numWin[nCntPlayer].pos.y , 0.0f);
			pVtx[2].pos = D3DXVECTOR3(g_numWin[nCntPlayer].pos.x  + nCntDigit*POLYGON_SIZE_X, g_numWin[nCntPlayer].pos.y + g_numWin[nCntPlayer].size.y, 0.0f);
			pVtx[3].pos = D3DXVECTOR3(g_numWin[nCntPlayer].pos.x  + g_numWin[nCntPlayer].size.x + nCntDigit*POLYGON_SIZE_X, g_numWin[nCntPlayer].pos.y + g_numWin[nCntPlayer].size.y, 0.0f);
...
{
	g_numWin[nPlayerIdx].score += value;

	if(g_numWin[nPlayerIdx].score >= NUM_DIGIT) g_numWin[nPlayerIdx].score = NUM_DIGIT; //最大値

	int number = g_numWin[nPlayerIdx].score;
	for(int keta = NUM_DIGIT; keta >= 1; keta--)
	{
		if(number > 0)
		{
			SetTextureNumWin(nPlayerIdx , keta , 1);
			number--;
		}		
		else
		{
			SetTextureNumWin(nPlayerIdx , keta , 0);
		}
	}
}
...
int GetWinner(void)
{
	for(int nCntPlayer = 0; nCntPlayer < MAX_PLAYER; nCntPlayer++)
	{
		if(g_numWin[nCntPlayer].score == NUM_GAME_SET)
		{
			return nCntPlayer;
		}
	}
